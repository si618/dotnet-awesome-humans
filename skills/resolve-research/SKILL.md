...
This is **editor-only work, so do not fan it out**. The wide, shallow sweeping already happened during the research; what remains is exactly the opinion-shaping judgment that [AGENTS.md](../../AGENTS.md#orchestration-and-model-economy) reserves for the strongest available model. If a claim needs re-checking, that is a `research-topic` re-run, not a worker sweep.
...
3. **Gate every claim the recommendation rests on against the roster.** Walk the citations:
   - **Citable roster ids:** promotable, and their ids go into the target's `sources:` frontmatter.
   ...
   - **References:** a named package's own documentation, release notes, issues and PRs, or the standard behind a named format. They promote as inline links that corroborate a roster-sourced claim, never into `sources:` and never as the only support for a claim ([AWESOME-HUMANS.md: Admission criteria](../../AWESOME-HUMANS.md#admission-criteria)). They are not blog-shaped sources and do not belong on the roster.
...
8. **Update metadata** on every file touched (YAML frontmatter on an opinion, the first-line comment header on a template): add the promoted source ids to `sources:`, and bump `last-reviewed`, but only once verified (see Rules).
...
- **Promotion ends in deletion, in the same pull request as the weave.** A promoted topic left on disk is a second, unratified copy of the opinion, and deletion is the only thing that marks it promoted. `audit-freshness` reports a lingering one. Git history is the record.
...
- **`last-reviewed` means verified against sources, not copied across.** If the research is inside the freshness tolerance, its verification carries and today's date is accurate. If it is not, re-verify first (see Edge cases). Bumping the date on the strength of stale research records exactly the false verification the metadata exists to prevent.
...
- **It is stale** (past the tolerance `audit-freshness` applies, or a major .NET release has shipped since): re-verify the claims it rests on against their sources before promoting, and check whether `refresh-dotnet-versions` should run first. Research that has aged past a release boundary is an input again, not a finished decision.
- **Only part of it can promote.** Promote the roster-sourced part and rewrite the rest down to the blocked remainder, leaving the file in place with a note saying what promoted and what it is waiting on. Do not let one unvetted paragraph block a sound recommendation, and do not delete the blocked part silently.
- **Everything in it is unvetted**: it cannot promote. Run `vet-source` on the strongest candidate and revisit, or discard.
- **It covers a preview feature**: it cannot become an opinion, because the freshness policy allows only a clearly marked "coming next" aside. Promote that aside if it is worth having, otherwise leave it open until GA.
