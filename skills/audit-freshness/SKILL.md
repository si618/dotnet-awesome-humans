...
| `research/` topics (all of them: a topic on disk is unresolved)                            | 90 days — then reported as promote-or-discard triage, resolved with `resolve-research`; one left on disk after promotion is itself a finding (promotion ends in deletion)                                         |
| `sources:` ids                                                                             | must all exist in `AWESOME-HUMANS.md` roster tables, or be the reserved `house` id (never a finding). `research/` topics only: unvetted source names are permitted when flagged as such in the text               |
...
| `templates/` build and test clean                                                          | zero — assemble the exemplar projects into the layout `example.slnx` describes and run `dotnet build` + `dotnet test`. Analyzer and package-compatibility failures only appear here, never from reading the files |
