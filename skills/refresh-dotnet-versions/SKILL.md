...
4. **Update template files** under `templates/`: TFMs in exemplar projects and anything version-pinned. Then the two pins that drift independently of a .NET release:
   - **4a. The SDK band in both `global.json` files.** Pin `sdk.version` to the **latest released SDK feature band** (e.g. `10.0.400`), not a patch-level build — with `"rollForward": "latestFeature"` the band is the floor, and pinning a patch needlessly fails machines one servicing release behind. Prefer `latestFeature` roll-forward wherever the opinion says so. This applies to `templates/global.json` **and** the repository's own root `global.json`, which moves with it: that one is the SDK the `scripts/` checks actually run on, so it may go up but never below `10.0.400`, the band that understands `#:include`. `global.json` cannot carry a comment header, so it has no `last-reviewed` of its own: the pinned value **is** the record, and step 8's PR description must state what it was checked against. Renovate never touches this field: `renovate.json` disables the `dotnet-sdk` depType, because a floor guarded by `latestFeature` has no bump worth making mechanically. This skill is the only thing that moves it.
