...
| 2026-09-04 | Named the date field in the `ms-learn` citation rule (no tier change) | The rule said to quote "the page's own review date" without naming it, and the `module-boundaries-and-events` research topic followed it into six wrong dates by quoting `updated_at`, a build timestamp; one of them became that pass's lead claim. The rule now names `ms.date` and says what `updated_at` is. The row's own example was wrong the same way and is corrected |
