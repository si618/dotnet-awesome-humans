...
Opinionated best practices for modern .NET software development in the AI-agentic era.
...
2. **Scaffolding and verification.** Copy-paste-ready reference files (`.editorconfig`, `Directory.Packages.props`, `Directory.Build.props`, `.csproj`, `.slnx`, `.slnf`, `global.json`, and friends) that encode the opinions, so new projects start right and existing projects can be verified against them.
3. **A concise guide for humans.** Each opinion is written to be skimmable — the opinion first, the rationale second, the sources last.

## Scope

The whole gamut of modern .NET:
...
Opinions must be earned. Every opinion traces back to sources with a **proven track record** — Microsoft's own canonical output and standout community members who have demonstrated a decade-class history of quality. These are catalogued in [AWESOME-HUMANS.md](AWESOME-HUMANS.md), including the admission criteria for joining the club.
...
├── opinions/                 ← the opinions, one topic per file
...
│   ├── testing.md
│   └── ...
├── reference/                ← copy-paste-ready example files
│   ├── .editorconfig
...
Skills are designed for a cost-aware split: **lower-cost subagents** fan out to do the web searches and source sweeps, while the strongest available model acts as the **oracle** — the orchestrator that synthesizes findings and is the only one that updates the actual opinions and reference files.

| Skill                     | Purpose                                                                                       |
| ------------------------- | --------------------------------------------------------------------------------------------- |
| `refresh-dotnet-versions` | Detect new .NET / C# / F# releases and update all opinions and reference files to target them |
| `harvest-awesome-humans`  | Sweep the awesome-humans sources for new posts and fold notable guidance into the opinions    |
| `vet-awesome-human`       | Evaluate a candidate source against the track-record criteria and admit or decline            |
| `audit-freshness`         | Report resources whose `last-reviewed` date has drifted past tolerance                        |
| `verify-project`          | Check an external project against the reference files and report deviations                   |

### Release watch automation

A scheduled GitHub Action (`.github/workflows/dotnet-release-watch.yml`) polls the official [.NET releases index](https://github.com/dotnet/core/blob/main/release-notes/releases-index.json) daily and compares it against the checked-in snapshot at `.github/state/dotnet-releases.json`. When a new GA release lands (runtime, ASP.NET Core, SDK), it opens a pull request updating the snapshot. That PR is the **trigger** for running `refresh-dotnet-versions` — it does not itself update any opinions or reference files.

## Using this repository

- **As an agent instruction:** "Follow the conventions in <https://github.com/…/dotnet-awesome-humans> when writing .NET code."
- **As scaffolding:** copy files from `reference/` into a new repository.
