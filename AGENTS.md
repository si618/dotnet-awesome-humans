...
- **Work in a git worktree, one branch per directory.** The local clone is bare, with each branch checked out beside it, so moving between pieces of work is a `cd` rather than a `git checkout`. Prefer this over switching branches in place: an in-place switch swaps the tree out from under anything holding a path into it — a running `dotnet` build, a formatter, another agent mid-edit — and discards the gitignored state (`node_modules/`, `artifacts/`, `bin/`, `obj/`) that the checks on the branch you left had already paid for. Add a worktree with `git worktree add <dir> <branch>` (`-b <branch>` to start a new one), naming the directory after the branch with **slashes flattened to dashes** — `resolve/globalisation` becomes `resolve-globalisation/`. That keeps every worktree one flat level beside the bare `.bare/` directory, tab-completable in a single step, and avoids a nested `research/` that is not the repository's own `research/`. When a worktree's pull request merges, **offer to delete that worktree** — `git worktree remove <dir>` and `git branch -d <branch>` — so stale checkouts do not accumulate; ask rather than assume, since the branch may still hold work that never reached the pull request. A fresh worktree carries no gitignored local setup, so run `npm install` in it before `npm run check`, and re-create the `.claude/skills` symlink there if your harness needs one (see [CLAUDE.md](CLAUDE.md)).
