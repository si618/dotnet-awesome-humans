...
- **A new file under `opinions/` is not finished until `README.md` lists it** — add a bullet under [Scope](README.md#scope) linking the file (`- **[Title](opinions/<file>.md)** — one line on what it covers`) and an entry in the [Repository layout](README.md#repository-layout) tree, kept alphabetical. Renaming or removing an opinion file means updating both places in the same commit. The `Validate skills and opinions` workflow enforces this, so a missed entry fails the pull request rather than drifting silently. The same applies to new files under `skills/`: add a row to the [Maintenance via skills](README.md#maintenance-via-skills) table.
