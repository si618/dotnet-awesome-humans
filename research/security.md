...
| `milan-jovanovic`  | Citable, COI noted           | Token storage, JWT validation, permissions, identity providers                                                                                                                                               |
| `code-with-mukesh` | Citable, **Corroborate.**    | Token lifetime, API keys, rate limiting across replicas, permission revocation                                                                                                                               |
| `khalid`           | Citable, pre-2025 posts only | Sharing Data Protection keys across apps (2022)                                                                                                                                                              |
| `aspnet-blog`      | Citable                      | Swept with `dotnet-blog`. No BFF or Data Protection post in 2024–2026 (negative result)                                                                                                                      |
| `jetbrains-dotnet` | Citable                      | Discovery only: the September 2026 roundup linked out to Lock, Khalid and Damien Bowden. No original security writing (negative result)                                                                      |
...
It fits ci.md's signing section, which already argues for provenance over author-signing ceremony. One citable author is thin for an opinion, though, and nothing swept says who consumes the SBOM.
...
7. **`vet-source` candidates, in order of what they would unlock:**
   ...
   - **Duende Software as a publication:** decline, or watch-list at most. It is product documentation for the library it sells, which is the independence cap's exact case.
