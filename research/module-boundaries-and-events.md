...
**Not confirmed, and partly contradicted: the vertical slice opinions.** Three of the five opinions in `architecture.md` describe the inside of a module, and `ms-learn` has nothing to say about any of them. It never uses "vertical slice" or "modular monolith", it offers no position on slices versus modules, and its worked example organises a solution by layer into Application Core, Infrastructure and UI projects. It then calls that arrangement "the most appropriate way to structure non-trivial monolithic applications", which is a solution-wide prescription where `architecture.md` argues for a per-slice one. The conflict is weighed in [Where the sources disagree](#where-the-sources-disagree) below.
...
`ms-learn` names the cost of the other choice plainly: "if there's an issue and the event handlers cannot commit their side effects, you'll have inconsistencies between aggregates", recoverable only by storing the events and running a reconciliation batch.
...
**Scope note on this citation.** The Event Sourcing pattern page sits under `learn.microsoft.com/azure/architecture/`, where the `ms-learn` roster row used to point at `learn.microsoft.com/dotnet/` alone. The row was widened on 2026-08-24 to cover first-party technical documentation across `learn.microsoft.com`, the Azure Architecture Center included, following the `avalonia-blog` widening of 2026-08-20: same publisher and same editorial standard, so a clarification rather than a new admission. The page is citable, subject to the two rules the widened row records — cite for mechanism and pattern trade-offs rather than product marketing, and quote each page's own review date, because they age unevenly.
...
1. **The TODO in `architecture.md` cannot be fully discharged from the current roster, and should be rewritten to say which half is blocked.** Corroboration exists for the modular-monolith opinions and does not exist for the vertical-slice opinions. Leaving one undifferentiated TODO hides that.
...
3. **The roster's architecture gap is narrower than it was, but not closed.** `oskar-dudycz` was admitted on 2026-08-24 off the back of this topic, which leaves the roster carrying two specialists over this ground, both with a conflict of interest recorded — `milan-jovanovic` in Notes, `oskar-dudycz` in Notes and marked **Corroborate.** as well — beside a documentation set that stops at the module boundary. Nothing independent covers it. Remaining candidates for `vet-source`, all **unvetted**, with activity checked on 2026-08-24 and everything else left for the vetting itself:
   ...
   - **Derek Comartin** ([codeopinion.com](https://codeopinion.com/)) — active to 2026-08-19 with on-topic material, but the posts are companion text for YouTube videos rather than standalone articles. That is what removed `nick-chapsas` on 2026-08-23. Only worth vetting if individual posts turn out to stand on their own as writing.
4. **The uncorroborated half needs a second sweep, now that there is a source for it.** `oskar-dudycz` was admitted on 2026-08-24 (Tier 1, marked **Corroborate.**) off the back of this topic, and his "Vertical slices, their ownership and external dependencies" of 2026-08-10 lands directly on the two opinions `ms-learn` is silent about. Nothing in this topic rests on him — he was not swept — so the slice opinions stay uncorroborated here until that sweep happens. Note when it does that his vertical-slices example is TypeScript, and that the **Corroborate.** marking means he cannot carry an opinion alone.
