...
**Scope note on this citation.** The `ms-learn` roster row points at `learn.microsoft.com/dotnet/`, and the Event Sourcing pattern page sits under `learn.microsoft.com/azure/architecture/`. The owner decided on 2026-08-24 that the id covers the relevant paths across `learn.microsoft.com`, the Azure Architecture Center included, which follows the `avalonia-blog` widening of 2026-08-20: same publisher and same editorial standard, so a clarification rather than a new admission. **The roster row still has to be edited to say so, via `vet-source`, before an opinion cites this page.**
...
3. **The roster has a real architecture gap.** It carries one architecture specialist with a recorded conflict of interest, one `**Corroborate.**`-marked source, and a documentation set that stops at the module boundary. Candidates for `vet-source`, all **unvetted**, with activity checked on 2026-08-24 and everything else left for the vetting itself:
   - **Oskar Dudycz** ([event-driven.io](https://event-driven.io/)) — the closest fit. Substantial written articles, active to 2026-08-10, and the catalogue reaches both halves of the gap: "Vertical slices, their ownership and external dependencies" lands on the uncorroborated ground directly, and "Fixing bugs in Event Sourcing is hard, for real?" shows critical distance on his own specialism. Co-maintains Marten and Emmett, so expect the independence rule to apply as it did for `shay-rojansky`. Start date needs establishing; the visible archive suggests 2023, which would put him in the Tier 2 band rather than Tier 1.
...
4. **`ms-learn`'s roster row needs editing** to record the 2026-08-24 widening across `learn.microsoft.com`, per the scope note in section 5.
