...
**Scope caveat on this citation.** The `ms-learn` roster row points at `learn.microsoft.com/dotnet/` and describes the focus as .NET documentation and architecture e-books. The Event Sourcing pattern page sits under `learn.microsoft.com/azure/architecture/`, same publisher and different section, so it is arguably outside the id as written. The `avalonia-blog` widening of 2026-08-20 is the precedent for resolving this with a scope clarification rather than a new admission. **This must be settled by `vet-source` before any opinion cites the page.**
...
3. **The roster has a real architecture gap.** It carries one architecture specialist with a recorded conflict of interest, one `**Corroborate.**`-marked source, and a documentation set that stops at the module boundary. Four `vet-source` candidates, all **unvetted** and all with track records to verify:
   - **Kamil Grzybek** ([kamilgrzybek.com](https://www.kamilgrzybek.com/blog/series/modular-monolith)) — the Modular Monolith series covers precisely the uncorroborated ground: Integration Styles, Architecture Enforcement, Architectural Drivers, Domain-Centric Design. The single highest-value candidate for this TODO. Independent.
   - **Derek Comartin** ([codeopinion.com](https://codeopinion.com/)) — event-driven architecture, messaging and boundaries, with a notably critical streak ("Is Event-Driven Architecture Overkill for Most Apps?", "You're Not as Decoupled as You Think"). Sells a course, so expect a COI note rather than a cap.
   - **Oskar Dudycz** ([event-driven.io](https://event-driven.io/)) — event sourcing in .NET, Marten co-maintainer. Would need the independence rule applied, as with `shay-rojansky`.
   - **Jimmy Bogard** — originator of the vertical slice framing and of the deferred domain-events pattern that `ms-learn` quotes above. Currently reached only second-hand through Microsoft's citations of him.
4. **`ms-learn`'s scope needs settling** before the Event Sourcing page can be cited, per the caveat in section 5.
