...
**If the app formats dates, sorts text, or resolves a time zone, check the base image before you shrink it.** [project-structure.md](project-structure.md) steers container builds to `noble-chiseled` or `alpine` for size, and those are exactly the images that leave globalisation data out. The failure mode is identical in both cases: it passes on the developer's machine and throws in production, because what is missing is in the image, not the code.

- **`tzdata` is absent from Alpine images**, so `TimeZoneInfo.FindSystemTimeZoneById` throws `TimeZoneNotFoundException`. Add `RUN apk add --no-cache tzdata`, or move to a Debian/Ubuntu-based image. ([Gordon — TimeZoneNotFoundException in Alpine Based Docker Images](https://www.stevejgordon.co.uk/timezonenotfoundexception-in-alpine-based-docker-images))
- **ICU is absent from the chiseled variants without `-extra`, and from `-composite`.** Those images only work for apps already in invariant mode. ([.NET container images](https://learn.microsoft.com/dotnet/core/docker/container-images))
