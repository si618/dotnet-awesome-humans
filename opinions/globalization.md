...
**Human-facing text uses the current culture; machine-facing text uses `CultureInfo.InvariantCulture` explicitly, and machine-facing comparison uses `StringComparison.Ordinal`.** This one rule prevents most globalization bugs, and the second half is the half that gets skipped — the failure is silent until someone runs the service under `de-DE` and `1.5` round-trips as `15`. ([Microsoft Learn: Best practices for comparing strings](https://learn.microsoft.com/dotnet/standard/base-types/best-practices-strings), [Microsoft Learn: Double.Parse](https://learn.microsoft.com/dotnet/api/system.double.parse))
...
- **Middleware order matters.** `UseRequestLocalization` runs before anything that reads the culture, and _after_ routing if you use `RouteDataRequestCultureProvider`. ([Globalization and localization in ASP.NET Core](https://learn.microsoft.com/aspnet/core/fundamentals/localization?view=aspnetcore-10.0))
