...
**Machine-generated timestamps are instants, so store them as UTC. Human-supplied future or recurring events are not instants until zone rules that may still change are applied, so store the local time and the IANA zone id, and treat UTC as derived.** ([Skeet: Storing UTC is not a silver bullet](https://codeblog.jonskeet.uk/2019/03/27/storing-utc-is-not-a-silver-bullet/))
...
- EF Core 8+ maps `DateOnly` ↔ SQL Server `date` and `TimeOnly` ↔ `time`, and scaffolding generates those types instead of `DateTime`/`TimeSpan` ([.NET Blog: EF Core 8 Preview 1: Raw, lazy, and on-time](https://devblogs.microsoft.com/dotnet/announcing-ef8-preview-1/)). The old `DateTime`-for-a-date mapping is a legacy shape.
...
This is enforced, but by a pair of files, not one. [templates/Directory.Build.props](../templates/Directory.Build.props) ships Meziantou.Analyzer with `TreatWarningsAsErrors`, and the date/time rules below ship at **info** severity or disabled, so on their own they never trip it. It is [templates/.editorconfig](../templates/.editorconfig)'s `dotnet_analyzer_diagnostic.severity = warning` that raises them to warnings, which `TreatWarningsAsErrors` then turns into build failures. Adopt both files, or a hand-rolled `IClock` builds clean. ([Meziantou: Meziantou.Analyzer rules](https://github.com/meziantou/Meziantou.Analyzer/blob/main/docs/README.md))
