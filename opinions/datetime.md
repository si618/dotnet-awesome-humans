...
This is enforced, not aspirational — but the enforcement is a pair, not one file. [templates/Directory.Build.props](../templates/Directory.Build.props) ships Meziantou.Analyzer with `TreatWarningsAsErrors`, and the date/time rules below ship at **info** severity or disabled, so on their own they never trip it; it is [templates/.editorconfig](../templates/.editorconfig)'s `dotnet_analyzer_diagnostic.severity = warning` that raises them to warnings, which `TreatWarningsAsErrors` then turns into build failures. Adopt both files, or a hand-rolled `IClock` builds clean ([analyzer docs](https://github.com/meziantou/Meziantou.Analyzer/blob/main/docs/README.md)):
