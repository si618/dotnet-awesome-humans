...
`shay-rojansky` carries a recorded independence limit — he maintains Npgsql and is on Microsoft's EF Core team, so the mapping rules above are cited as mechanism, which is what his notes permit. The judgment of when to reach for those types is `jon-skeet`'s in [datetime.md](datetime.md), not his.
